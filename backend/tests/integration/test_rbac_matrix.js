...
describe('FR-000/NFR-03/SC-006: matriz RBAC — 4 roles x endpoints restringidos por rol', () => {
  ...
  test('rol incorrecto -> 403 en cada endpoint restringido por rol', async () => {
    const restrictedEndpoints = [
      { allowedRole: 'dispatcher', build: (orderId) => (req) => req.post('/api/v1/orders').send({ clientId: 'x' }) },
      { allowedRole: 'dispatcher', build: (orderId) => (req) => req.patch(`/api/v1/orders/${orderId}/assign`).send({ technicianId: 'x', expectedVersion: 0 }) },
      { allowedRole: 'dispatcher', build: () => (req) => req.patch('/api/v1/technicians/x/activo').send({ activo: false }) },
      { allowedRole: 'tecnico', build: (orderId) => (req) => req.post(`/api/v1/orders/${orderId}/executions`).field('notes', 'x') },
      { allowedRole: 'supervisor', build: (orderId) => (req) => req.post(`/api/v1/orders/${orderId}/approve`).send({}) },
      { allowedRole: 'supervisor', build: (orderId) => (req) => req.post(`/api/v1/orders/${orderId}/reject`).send({ reason: 'x' }) },
    ];
