...
describe('NFR-02b/SC-007: fallo del encryptionAdapter/KMS al persistir una foto', () => {
  afterEach(() => {
    delete process.env.KMS_SIMULATE_FAILURE;
  });

  test('KMS no disponible -> 503, ninguna foto se persiste (ni en claro ni cifrada)', async () => {
    ...
    process.env.KMS_SIMULATE_FAILURE = 'true';
    const res = await request(app)
      .post(`/api/v1/orders/${order.id}/executions`)
      .set('Authorization', `Bearer ${token}`)
      .set('Idempotency-Key', 'kms-fail-1')
