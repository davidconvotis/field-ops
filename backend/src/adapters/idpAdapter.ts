...
function issueDevToken({ userId, role }: DevTokenPayload): string {
  const secret = process.env.JWT_DEV_SECRET;
 ...
 */
function issueTokenPair({ userId, role }: DevTokenPayload): TokenPair {
  const secret = process.env.JWT_DEV_SECRET;
 ...
 */
async function revokeRefreshToken({ jti, userId, expiresAt }: RevokeRefreshTokenInput): Promise<void> {
