import type { PrismaClient, ExecutionRecord, EvidencePhoto } from '@prisma/client';
...
interface ComputePayloadHashParams {
  notes: string;
  files: FileInput[];
}

interface SubmitExecutionParams {
  orderId: string;
  technicianId: string;
  notes: string;
  files: FileInput[];
...
interface SubmitExecutionResult {
  status: number;
  body: ExecutionResponse;
}

type ExecutionRecordWithPhotos = ExecutionRecord & { photos?: EvidencePhoto[] };
...
function computePayloadHash({ notes, files }: ComputePayloadHashParams): string {
 ...
 */
async function submitExecution({ orderId, technicianId, notes, files, idempotencyKey }: SubmitExecutionParams): Promise<SubmitExecutionResult> {
  ...
  if (detectedTypes.some((t: string | null) => t === null)) {
  ...
  try {
    ...
    if (name === 'StorageTimeoutError') throw new HttpError(504, 'timeout del almacenamiento de fotos excedido');
    if (name === 'StorageUnavailableError') throw new HttpError(502, 'almacenamiento de fotos no disponible');
    if (name === 'EncryptionUnavailableError') throw new HttpError(503, 'servicio de cifrado no disponible');
    throw err;
  }
...
function toExecutionResponse(record: ExecutionRecordWithPhotos): ExecutionResponse {
  return {
    id: record.id,
...
export = { submitExecution, HttpError };
