...
import { listOrders, createOrder, assignOrder, ApiError } from '../services/api.js';
import TechnicianAssignSelect from '../components/TechnicianAssignSelect.jsx';
import ReassignConfirmModal from '../components/ReassignConfirmModal.jsx';
...
export default function DispatcherOrders() {
  ...
  const [pendingReassign, setPendingReassign] = useState(null); // { order, newTechnicianId }
  ...
  function handleFilterChange(nextStatus, nextTechnicianId) {
  ...
  function handleAssignSelected(order, selectedTechnicianId) {
  ...
  async function doAssign(order, selectedTechnicianId, expectedVersion) {
    try {
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-wrap items-end gap-4 rounded-lg bg-white p-4 shadow-sm">
        <label className="block text-sm text-slate-600">
          ...
          <select
            value={status}
            onChange={(e) => handleFilterChange(e.target.value, technicianId)}
                ...
                <tr key={order.id} className="border-b border-slate-100 text-sm">
                  ...
                  <td className="p-3">
                    {TERMINAL_STATUSES.includes(order.status) ? (
                      ...
                      <TechnicianAssignSelect onSelect={(techId) => handleAssignSelected(order, techId)} />
                    )}
                  </td>
                </tr>
              ))}
      ...
      {pendingReassign && (
        <ReassignConfirmModal
          previousTechnicianNombre={pendingReassign.order.technicianNombre}
